    ...
    <div className="min-h-screen bg-zinc-950 text-zinc-100">
      <header className="sticky top-0 z-20 flex items-center justify-between border-b border-white/[0.06] bg-zinc-950/80 px-8 py-4 backdrop-blur-md">
        <div className="flex items-center gap-3">
          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-white/[0.06]">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
              <path d="M4 4h16v10H7l-3 3V4z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
            </svg>
          </div>
          <div>
        ...
        <div className="flex items-center gap-3 text-xs text-zinc-500">
          {selectedCollection ? (
            <span className="rounded-full bg-white/[0.05] px-3 py-1 text-[11px]">
              Collection <span className="font-semibold text-zinc-200">{selectedCollection}</span>
            </span>
          ) : (
            <span className="rounded-full bg-white/[0.03] px-3 py-1 text-[11px]">
      ...
      <div className="mx-auto flex max-w-6xl gap-6 px-6 py-6">
        <div className="flex flex-1 flex-col rounded-2xl border border-white/[0.06] bg-zinc-950/90 shadow-[0_18px_45px_rgba(0,0,0,0.55)]">
          <div className="flex items-center gap-3 border-b border-white/[0.06] px-5 py-3 text-xs">
          <select
            value={selectedCollection}
            onChange={(e) => setCollection(e.target.value)}
              className="rounded-lg border border-white/[0.08] bg-white/[0.02] px-3 py-1.5 text-[12px] text-zinc-200 outline-none focus:border-white/[0.2]"
          >
          ...
          <label className="flex items-center gap-1 text-xs">
            ...
            <input
              ...
              className="w-14 rounded-lg border border-white/[0.08] bg-white/[0.02] px-2 py-1 text-[11px] text-zinc-200 outline-none focus:border-white/[0.2]"
            />
          </label>
          <button
            type="button"
            onClick={clearMessages}
            className="ml-auto rounded-lg border border-white/[0.08] bg-white/[0.02] px-3 py-1.5 text-[11px] font-medium text-zinc-300 transition hover:bg-white/[0.08]"
          >
            Clear chat
          </button>
        </div>
            ...
            >
              <div
                className={`max-w-xl rounded-2xl px-3.5 py-2.5 text-[13px] leading-relaxed ${
                  m.role === "user"
                    ? "bg-primary/90 text-primary-foreground rounded-br-sm shadow-[0_10px_30px_rgba(0,0,0,0.45)]"
                    : "bg-zinc-900/90 text-zinc-100 border border-white/[0.04] rounded-bl-sm"
                }`}
              >
                <div>{m.content}</div>
                {m.latency && (
          ...
          >
            <input
              className="flex-1 rounded-xl border border-white/[0.08] bg-zinc-950/80 px-3.5 py-2.5 text-sm outline-none ring-0 placeholder:text-zinc-600 focus:border-primary focus:ring-2 focus:ring-primary/40"
              placeholder={
                selectedCollection
            ...
            <button
              ...
              className="inline-flex items-center justify-center rounded-xl border border-white/[0.06] bg-white px-4 py-2.5 text-sm font-medium text-zinc-950 shadow-sm transition hover:bg-zinc-200 active:scale-[0.97] disabled:cursor-not-allowed disabled:border-white/[0.02] disabled:bg-white/40"
            >
              Send
            </button>
          </form>
        ...
        <div className="hidden w-80 flex-col rounded-2xl border border-white/[0.06] bg-zinc-950/90 p-4 text-xs shadow-[0_18px_45px_rgba(0,0,0,0.55)] md:flex">
